...
class BBoxVisualizer:
    ...
    def update_ego_bbox(self, ego_pose):
        ...
        if ego_pose == None:
            logger.error(f"No ego_pose for the current frame.")
            return
    ...
    def visualize(self, point_cloud, detections, ego_pose):
        ...
        # camera
        if ego_pose == None:
            cam_eye_world  = self.cam_eye_local
            cam_look_world = self.cam_look_local
        else:
            ex, ey, ez = np.array([ego_pose['x'], ego_pose['y'], ego_pose['z']])
