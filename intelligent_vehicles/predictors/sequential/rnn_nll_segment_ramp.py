...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~  train  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
    def train(self, train_loader, valid_loader=None, save_path=None):
        """
        Mixture of losses:
          clean + {global FGSM, unified segment-ramp (covers A′ anywhere & A tail via sampling)}
        - ε_base comes from cfg["epsilon"] (default 0.03)
        - segment length L ∈ [1 .. min(5, ceil(0.35*T))]
        - per-step ramp 1×..min(L, m_cap)× with m_cap ∈ [1..5]
        """
        print("Train batches:", len(train_loader))
    
        eps_base   = getattr(self, "epsilon", 0.03)      # base ε (data units)
        p_tail     = getattr(self, "p_tail", 0.5)        # prob to place segment at tail
        w_clean    = getattr(self, "w_clean", 0.5)
        w_global   = getattr(self, "w_global", 0.25)
        w_seg      = getattr(self, "w_seg", 0.25)
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            for cat, obs, tgt in bar:
                ...
                # ----- Clean forward (and grad wrt inputs) -----
                ...
                # (2) Unified segment-ramp (anywhere vs tail via p_tail; ramp 1..5×)
                ...
                # ----- Weighted mixture -----
                total_loss = w_clean * loss_clean + w_global * loss_g + w_seg * loss_s
    
                self.optimizer.zero_grad()
                total_loss.backward()
                self.optimizer.step()
    
                # ----- Logging / metrics from clean pass -----
