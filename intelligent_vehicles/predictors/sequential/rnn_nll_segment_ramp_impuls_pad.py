...
class RNNPredictorNLL:
    ...
    @staticmethod
    def _short_history_variant(enc_in, min_len=2, max_frac=0.5, p_very_short=0.3):
        """
        Create a short-history version by keeping only the last N valid steps and left-padding zeros.
        enc_in: [B, T-1, D]
        N is sampled per sample:
          - with prob p_very_short: N in {2,3,4} (clipped by T-1)
          - else: uniform in [min_len .. floor(max_frac*(T-1))]
        """
        B, Tm1, D = enc_in.shape
        device = enc_in.device
        enc_short = torch.zeros_like(enc_in)  # left-padded zeros by default
    
        if Tm1 <= 1:
            return enc_short  # degenerate window
    
        # bounds for "generic" short lengths
        max_len = max(min_len, int(Tm1 * max_frac))
        max_len = min(max_len, Tm1)
    
        # sample per-sample N
        Ns = torch.empty(B, dtype=torch.long, device=device)
        very_short_mask = torch.rand(B, device=device) < p_very_short
        # very short: choose from {2,3,4} (clipped by Tm1)
        if very_short_mask.any():
            choices = torch.tensor([2, 3, 4], device=device)
            Ns_vs = choices[torch.randint(0, len(choices), (very_short_mask.sum(),), device=device)]
            Ns_vs = torch.clamp(Ns_vs, 1, Tm1)
            Ns[very_short_mask] = Ns_vs
        # generic: uniform in [min_len .. max_len]
        gen_mask = ~very_short_mask
        if gen_mask.any():
            low = max(1, min_len)
            high = max(low + 1, max_len + 1)  # randint upper is exclusive
            Ns_g = torch.randint(low=low, high=high, size=(gen_mask.sum(),), device=device)
            Ns[gen_mask] = Ns_g
    
        # place last N steps at the tail, zeros in the prefix
        for b in range(B):
            N = int(Ns[b].item())
            N = max(1, min(N, Tm1))
            enc_short[b, Tm1 - N : Tm1] = enc_in[b, Tm1 - N : Tm1]
    
        return enc_short.detach()


    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~  train  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
    def train(self, train_loader, valid_loader=None, save_path=None):
        """
        Mixture: clean + {global FGSM, unified segment-ramp (A′/A), impulse(+short tail)} + short-history pass
        - short-history pass: zero-pad left, keep only last N valid steps (separate, small weight)
        """
        print("Train batches:", len(train_loader))
    
        eps_base   = getattr(self, "epsilon", 0.03)   # base ε (data units)
        p_tail     = getattr(self, "p_tail", 0.5)     # prob segment at tail
        w_clean    = getattr(self, "w_clean", 0.5)
        w_short    = getattr(self, "w_short", 0.10)   # weight for short-history loss
    
        # split remaining weight across 3 adversaries
        rem = max(0.0, 1.0 - w_clean - w_short)
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            for cat, obs, tgt in bar:
                ...
                # ----- short-history (structural, separate pass) -----
                with torch.no_grad():
                    enc_short = self._short_history_variant(enc_in.detach(), min_len=2, max_frac=0.5, p_very_short=0.3)
                mu_sh, lv_sh = self.model(enc_short, tgt_v.size(1))
                loss_short   = self.gaussian_nll(tgt_v, mu_sh, lv_sh).mean()
                ...
                with torch.no_grad():
                    ...
                    logv_sum += logv_mean
                    lv_min = logv_c.min().item()
                    lv_max = logv_c.max().item()
                    if ep_loss == total_loss.item():
                        logv_min, logv_max = lv_min, lv_max
                    else:
                        logv_min = min(logv_min, lv_min) 
                        logv_max = max(logv_max, lv_max)
